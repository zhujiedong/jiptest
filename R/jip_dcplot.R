...
#' @import scales
# @export
jip_dcplot <- function(file_dir) {
  read_dcfiles <- match.fun(read_dcfiles)
  ojipdc <- read_dcfiles(file_dir)

  dcplot <- ggplot(ojipdc, aes(SECS, FLUOR, group = SOURCE, colour = SOURCE)) +
    geom_point(aes()) + labs(x = "Time (secs)",
                             y = "fluoresence signal") + scale_colour_hue(name =
                              "Data files", labels = unique(as.character(ojipdc$SOURCE))) +
    scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                  labels = trans_format("log10", math_format(10^.x))) +
    ggtitle("DC fluorescence")
    dcplot
}
