...
#'
#' @export
jip_comp <- function(jip_data, normalized = FALSE) {
  if(!normalized){
  ...
  #return(para_values)
    ...
    Fo <- min(jip_data$NORM_FLUOR)
    To <- jip_data[which.min(jip_data$NORM_FLUOR), ]
    Tm <- jip_data[which.max(jip_data$NORM_FLUOR), ]
    Fm <- max(jip_data$NORM_FLUOR)
    TJ <- jip_data[which(jip_data$SECS > 0.0018 & jip_data$SECS < 0.0022), ]
    FJ <- max(TJ$FLUOR)
    TI <- jip_data[which(jip_data$SECS > 0.028 & jip_data$SECS < 0.032), ]
    FI <- max(TI$FLUOR)
